...
csmInt32 lastPlayed = -1;
csmInt32 maxPlayed = -1;
csmString lastAnimation = "";
...
void LAppModel::SetupModel(ICubismModelSetting *setting) {
    ...
    _modelMatrix->Scale(LAppDefine::scaleSize, LAppDefine::scaleSize);
    if (LAppDefine::centerModel)
        _modelMatrix->SetPosition(LAppDefine::centerModelX, LAppDefine::centerModelY);
...
void LAppModel::PreloadMotionGroup(const csmChar *group) {
    ...
    for (csmInt32 i = 0; i < count; i++) {
        ...
        csmSizeInt size;
        buffer = CreateBuffer(path.GetRawString(), &size);
        CubismMotion *tmpMotion = static_cast<CubismMotion *>(LoadMotion(buffer, size,
                                                                         name.GetRawString()));
...
void LAppModel::SetupTextures() {
    for (csmInt32 modelTextureNumber = 0;
         modelTextureNumber < _modelSetting->GetTextureCount(); modelTextureNumber++) {
...
void FinishedMotion(ACubismMotion *self) {
    lastPlayed++;
    if (lastPlayed < maxPlayed) {
        LAppLive2DManager::GetInstance()->GetModel()->StartMotion(lastAnimation.GetRawString(),
                                                                  lastPlayed, PriorityForce,
                                                                  FinishedMotion);
        ...
        maxPlayed = -1;
        lastAnimation = "";
        LAppPal::PrintMessage("Motion Group Finished");
    }
}

void LAppModel::PlayAnimation(std::string aniname) {
    if (_modelSetting->GetMotionCount(aniname.c_str()) == 0) {
        return;
    }
    lastAnimation = aniname.c_str();
    maxPlayed = _modelSetting->GetMotionCount(aniname.c_str());
    lastPlayed = 0;
    StartMotion(aniname.c_str(), lastPlayed, PriorityForce, FinishedMotion);
}


std::vector<std::string> LAppModel::GetExpressionList() {
    std::vector<std::string> face;
    auto it = _expressions.Begin();
    while (it != _expressions.End()) {
        face.push_back(it->First.GetRawString());
        it++;
    }
    return face;
}

std::vector<std::string> LAppModel::GetMotionList() {
    std::vector<std::string> motions;
    auto it = _motions.Begin();
    while (it != _motions.End()) {
        motions.push_back(it->First.GetRawString());
        it++;
    }
    return motions;
}

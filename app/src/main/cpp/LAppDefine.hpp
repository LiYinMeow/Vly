...
namespace LAppDefine {
    ...
    using namespace Csm;
    extern bool configMode; //背景画布透明度（调试用）
    ...
    // モデル定義--------------------------------------------
    extern std::string ModelName;               ///< モデルを配置したディレクトリ名の配列. ディレクトリ名とmodel3.jsonの名前を一致させておく.
    extern csmFloat32 scaleSize;              //模型缩放
    extern const bool centerModel;
    extern csmFloat32 centerModelX;
    extern csmFloat32 centerModelY;
    // 外部定義ファイル(json)と合わせる
